...
% limitations of Boids; artificial potential fields
...
We can reduce this reaction time and the possibility of steady-state error by attempting to reduce the area between the $\theta_d$ and the actual angle (the curve in the graphs). Namely, we can introduce an integral term, which is proportional to the area between the $\theta_d$ and the actual angle. This is shown in Eq. \ref{eq:pid-controller}.
...
Figure \ref{fig:exp1} shows the x-z positions of the agents within the flock over a simulated run with these constants. We see that the agents move together in a straight line, signifying that the alignment weight is initially well-tuned. We note that the behaviour of some agents is not as is desired, as in Fig. \ref{fig:exp1}, we see there exists a large gap between two agents. This is likely due to the avoidance weight being too low, causing the agents to not move closer to each other. As a result, we will increase the avoidance weight to \textbf{1.5} and re-run the simulation. We will also increase the neighbourhood radius to \textbf{3} to mitigate issues with agents not being aware of each other.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \addplot table [x expr=\coordindex, y=tcost, col sep=comma] {altcooling.csv};
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \addplot table [x expr=\coordindex, y=tcost, col sep=comma] {size500.csv};
...
\begin{figure}[ht]
   ...
   \end{tikzpicture}
   \caption{Sensitivity Analysis with Increased Resolution}
   \label{fig:spiderweb-resolution}
   \end{figure}

We could choose to explore the sensitivity analysis further, by performing an analysis of the sensitivity of multiple axes (behaviours) simultaneously, to discover links between the behaviours. In reality, with five behaviours, this would be a 5-dimensional space, which would be challenging to visualise. However, this could provide valuable insights into the relationships between the behaviours and the cost function. For these reasons, it is omitted from this study; however, in the next section, we explore a simplification of the parameter space to reduce the dimensionality of the problem.
